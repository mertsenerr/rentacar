...
public class VehiclesController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<IEnumerable<VehicleAvailabilityDto>>>> GetAvailability(
        [FromQuery] string pickupDate, [FromQuery] string returnDate)
    {
        var pickup = DateTime.SpecifyKind(DateTime.Parse(pickupDate), DateTimeKind.Utc);
        var returnD = DateTime.SpecifyKind(DateTime.Parse(returnDate), DateTimeKind.Utc);
        var vehicles = await _vehicleService.GetAllWithAvailabilityAsync(pickup, returnD);
        return Ok(new ApiResponse<IEnumerable<VehicleAvailabilityDto>>(true, vehicles, null, null, null));
    }
...
public class BookingsController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<BookingDto>>> Create([FromBody] BookingRequest request)
    {
        ...
        {
            return Conflict(new ApiResponse<BookingDto>(false, null, ex.Message, null, null));
        }
    }

    [HttpPost("availability")]
    public async Task<ActionResult<ApiResponse<AvailabilityResponse>>> CheckAvailability([FromBody] AvailabilityRequest request)
    {
        var startDate = DateTime.SpecifyKind(DateTime.Parse(request.StartDate), DateTimeKind.Utc);
        var endDate = DateTime.SpecifyKind(DateTime.Parse(request.EndDate), DateTimeKind.Utc);
        var available = await _bookingService.CheckAvailabilityAsync(request.VehicleId, startDate, endDate);
        var message = available ? null : "This vehicle is already booked for the selected dates. Please choose different dates.";
        var response = new AvailabilityResponse(available, message);
        return Ok(new ApiResponse<AvailabilityResponse>(true, response, null, null, null));
    }
